...
class TestAWDBPerformance:
    ...
    @pytest.mark.asyncio
    async def test_bulk_station_retrieval(self, client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_station_queries(self, client):
        """Test concurrent station queries."""
        networks = ['SCAN', 'SNTL', 'COOP', 'USGS']

        async def concurrent_test():
            async def query_network(network):
                return await client.get_stations(network_codes=[network], active_only=True)
    ...
    @pytest.mark.asyncio
    async def test_data_retrieval_performance(self, client):
    ...
    @pytest.mark.asyncio
    async def test_api_parameter_combinations(self, client):
...
class TestAWDBBulkOperations:
    """Test bulk data operations and batch processing."""

    @pytest.fixture
    def client(self):
        """Create AWDB client for testing."""
        return AWDBClient(timeout=60)  # Longer timeout for bulk operations

    @pytest.mark.asyncio
    async def test_multi_station_data_retrieval(self, client):
    ...
    @pytest.mark.asyncio
    async def test_network_based_bulk_queries(self, client):
    ...
    @pytest.mark.asyncio
    async def test_error_recovery_in_bulk_operations(self, client):
...
class TestAWDBDataAvailabilityScanning:
    ...
    @pytest.mark.asyncio
    async def test_station_data_availability_scan(self, client):
        ...
        # Test functions should not return values, just assert
        assert True


class TestAWDBPaginationAndLargeDatasets:
    """Test pagination and large dataset handling."""

    @pytest.fixture
    def client(self):
        """Create AWDB client for testing."""
        return AWDBClient(timeout=60)  # Longer timeout for large operations

    @pytest.mark.asyncio
    async def test_large_station_result_handling(self, client):
        """Test handling of large station result sets."""
        # Get all stations (this will be a large result)
        async with client:
            try:
                start_time = time.time()
                all_stations = await client.get_stations(active_only=True)
                end_time = time.time()
                ...
                # This might fail due to API limits, so we'll make it non-critical
                pytest.skip(f"Large station retrieval not available: {e}")

    @pytest.mark.asyncio
    async def test_chunked_data_retrieval(self, client):
    ...
    @pytest.mark.asyncio
    async def test_memory_efficient_processing(self, client):
        ...
        async with client:
            ...
            try:
                ...
                if not data:
                    pytest.skip("No data available for memory efficiency test")
    ...
    @pytest.mark.asyncio
    async def test_api_rate_limit_adaptation(self, client):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_request_limits(self, client):
