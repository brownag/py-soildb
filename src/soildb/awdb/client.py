...
class AWDBClient:
    ...
    async def check_station_data_availability(
        ...
        Args:
            ...
            ordinal: Sensor ordinal (1, 2, 3, etc. for multiple sensors)
            period_ref: Timestamp reference ('START' or 'END')
            central_tendency_type: Central tendency ('NONE', 'ALL', 'MEDIAN', 'AVERAGE')
            return_flags: Include quality flags in response
            return_original_values: Include original values and flags
            return_suspect_data: Include suspect data (normally filtered out)
            insert_or_update_begin_date: Date for incremental updates
    ...
    async def get_reference_data(
        ...
        Args:
            reference_lists: List of reference data types to retrieve.
                            Options: dcos, durations, elements, forecastPeriods,
                            functions, instruments, networks, physicalElements,
                            states, units. If None, returns all.

        Returns:
            ReferenceData object containing all requested reference data
        """
        params = {}
        if reference_lists:
