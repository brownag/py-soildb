...
async def hierarchy_example():
    ...
    hz_response = await soildb.fetch_chorizon_by_cokey(cokeys)
    hz_df = hz_response.to_pandas()
    
    print("Soil Survey Hierarchy Analysis")
    print("="*40)
    ...
    if not hz_df.empty:
        ...
        if not clay_data.empty:
            print(f"Average clay content: {clay_data.mean():.1f}%")
    
    return mapunit_df, comp_df, hz_df

# Run the example
mu_df, comp_df, hz_df = await hierarchy_example()
```
