...
The `.sync` methods are intended for use in synchronous code only. They will raise a `SyncUsageError` if called from within an existing asyncio event loop. In such cases, use the async version instead.