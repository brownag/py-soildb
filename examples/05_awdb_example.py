...
def fetch_and_print_data(client, network_code, property_name, start_date="2023-01-01", end_date="2023-01-10", height_depth_inches=None, state_codes=None, max_stations=3):
    """
    Fetches and prints data for a given network and property.
    (Updated to use proper API element format - legacy function maintained for compatibility)
    """
    print(f"\n--- Example: Fetching {property_name} for {network_code} network ---")

    # Get stations for this network (now using API filtering)
    stations = client.get_stations(network_codes=[network_code], state_codes=state_codes)
    if not stations:
        print(f"No stations found for network: {network_code}")
        return

    print(f"Found {len(stations)} {network_code} stations total.")
    stations = stations[:max_stations]  # Limit to first few stations
    print(f"Testing data retrieval for first {len(stations)} {network_code} stations.")
